...
123 Wardens contributed reports to the Ajna Protocol:
    ...
    Eurovickk
    Evo
    GG_Security (georgits and halden)
    Haipls
    J4de
    JCN
    Jerry0x
...
The updateBucketExchangeRatesAndClaim function is publicly callable and serves two main purposes:
    ...
    Updates the exchange rate of a list of buckets.
    If eligible, the caller can claim 5% of the rewards accumulated to each bucket since the last burn event, if it hasn’t already been updated.
...
Consider the following scenario:
    ...
    Alice is hard-working, non-technical and constantly keeps track of when to update the buckets so she can claim a small reward. Unfortunately, she becomes notorious for getting most of the rewards from updating the bucket exchange rate.
    A malicious actor spots Alice’s recent gains and creates a bot to front run any transactions to RewardsManager’s _updateBucketExchangeRateAndCalculateRewardssubmitted by Alice.
    The day after that, Alice again see’s theres a small reward to claim, attempts to claim it, but she gets front runned by whoever set the bot.
    Since Alice is non-technical, she cannot ever beat the bot so she is left with a broken heart and no longer able to claim rewards.
...
I see potentially two solutions here:
    ...
    Introduce a randomized reward mechanism, such as a lottery system or a probabilistic reward distribution for people who contribute to updating buckets. This could reduce the predictability of rewards and hence the potential for MEV exploitation.
    Consider limiting the reward claim process to users who have staked in the rewards manager because they are the individuals that are directly affected if the bucket is not updated, because if its not updated for 14 days they won’t be getting rewards. Additionally, you can couple it with a rate-limitting mechanism by implementing a maximum claim per address per time period
...
The following steps can occur for the described scenario.
    ...
    At 1 block prior to the current distribution period’s start block, Alice has no available votes at all.
    After noticing the StandardFunding.startNewDistributionPeriod transaction that would end the previous distribution period and starts the current distribution period in the mempool, Alice backruns that transaction by minting a lot of AJNA tokens at the current distribution period’s start block.
    When block.number becomes bigger than the current distribution period’s start block during the screening phase, Alice calls the StandardFunding.screeningVote function to successfully use all of her available votes at the current distribution period’s start block for a proposal.
    Alice’s actions are unfair to Bob who prepares for the screening stage voting and maintains the same number of available votes at 1 block prior to the current distribution period’s start block and at the current distribution period’s start block.
...
Examples:
    ...
    If 65% of non-treasury tokens vote affirmatively for a proposal, up to 15% of the treasury may be withdrawn by the proposal
    If 50% or less of non-treasury tokens vote affirmatively for a proposal, 0% of the treasury may be withdrawn by the proposal
...
Example: a. A proposer requests 10% of the treasury
    ...
    50%+10%=60%
    If 65% of non-treasury tokens vote affirmatively, 10% of the treasury is released
    If 59.9% of non-treasury tokens vote affirmatively, 0% of the treasury is released
...
Two ways to fix this problem:
    ...
    FundingVote does not allow users who didn’t vote in screening stage.
    Delete the code on line L240.
...
Proof of Concept
    ...
    Create a file under test/unit/ and call it ExploitEF.t.sol
    ...
    Run forge test --match-contract ExploitEF --match-test test_drainTreasury
...
According to the specifications of the standard EIP-4494, the following violations were found:
    ...
    EIP-4494 requires the implementation of IERC165 and the indication of support for the interface 0x5604e225, which is not implemented
    EIP-4494 requires the presence of the function function nonces(uint256 tokenId) external view returns(uint256); which is missing
    EIP-4494 requires the function function permit(address spender, uint256 tokenId, uint256 deadline, bytes memory sig) external;, which is incorrectly declared as
